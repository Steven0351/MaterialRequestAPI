...
import WorkOrderCompletionRequest from '../model/work-order-completion-request';
...
export default({ config, db }) => {
  ...
  api.put('/:id', authenticate, (req, res) => {
    WorkOrderCompletionRequest.findById(req.params.id, (err, WorkOrderCompletionRequest) => {
      if (err) {
        res.status(500).send(err);
      } else if (req.body.requestor != workOrderCompletionRequest.requestor || req.body.requestor != '5920befd422aeb963bf0fee0') {
